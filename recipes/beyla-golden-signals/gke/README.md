...
Follow the the instructions to [view application metrics in
GKE](https://cloud.google.com/stackdriver/docs/managed-prometheus/exporters/server/http#view-dashboard).
You can comment out the `name != "http.server.duration"` filter clause in
[collector-config.yaml](collector-config.yaml) to send [all metrics generated by
Beyla](https://grafana.com/docs/beyla/latest/metrics/), which can be viewed in the Metrics
Explorer.
